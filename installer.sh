#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpScreenGrab/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpScreenGrab

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpScreenGrab
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpScreenGrab
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpScreenGrab/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpScreenGrab-main/usr' '/'
set +e
cd
sleep 2
...
echo ""
echo "#########################################################"
echo "#          CiefpScreenGrab INSTALLED SUCCESSFULLY       #"
